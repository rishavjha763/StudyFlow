...
const ACHIEVEMENTS = [
  { key: 'first_session', title: 'First Study Session', description: 'Complete your first study session', icon: 'clock' },
  { key: 'first_note', title: 'First Note', description: 'Create your first note', icon: 'edit' },
  { key: 'first_quiz', title: 'First Quiz', description: 'Complete your first quiz', icon: 'help-circle' },
  { key: 'streak_7', title: '7 Day Streak', description: 'Study 7 days in a row', icon: 'flame' },
  { key: 'streak_30', title: '30 Day Streak', description: 'Study 30 days in a row', icon: 'flame' },
  { key: 'hours_100', title: '100 Hours Studied', description: 'Reach 100 lifetime study hours', icon: 'award' },
  { key: 'notes_100', title: '100 Notes', description: 'Create 100 notes', icon: 'book' },
  { key: 'quiz_master', title: 'Quiz Master', description: 'Complete 10 quizzes', icon: 'star' }
];
...
async function checkAndUnlockAchievements(userId) {
  const already = await UnlockedAchievement.find({ user: userId }).select('key');
  ...
  const lifetimeSeconds = dailyStats.reduce((sum, d) => sum + d.totalStudySeconds, 0);
  ...
  for (const def of ACHIEVEMENTS) {
    if (!unlockedKeys.has(def.key) && conditionMet[def.key]) {
      ...
      const xpResult = await awardXP(userId, 'ACHIEVEMENT_UNLOCKED', `Unlocked: ${def.title}`);
