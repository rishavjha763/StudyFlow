...
import { FiMenu, FiTarget, FiX, FiClock } from 'react-icons/fi';
...
export default function AppLayout({ section = 'Workspace', title, children }) {
  ...
  const { focusMode, enterFocusMode, exitFocusMode } = useFocusMode();

  return (
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-950">
      {!focusMode && <Sidebar mobileOpen={mobileOpen} onClose={() => setMobileOpen(false)} />}
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                {timerStatus !== 'idle' && (
                  ...
                  >
                    <span className={`w-1.5 h-1.5 rounded-full bg-primary-500 ${timerStatus === 'running' ? 'animate-pulse' : ''}`} />
