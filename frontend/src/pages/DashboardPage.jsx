...
import { FiDownload, FiAward, FiHelpCircle, FiArrowRight, FiZap } from 'react-icons/fi';
...
export default function DashboardPage() {
  ...
  return (
    <AppLayout section="Workspace" title="Dashboard">
      ...
      >
        <motion.div variants={fadeUp} transition={{ duration: 0.35 }} className="flex justify-end">
          <button
            onClick={() => setDoubtSolverOpen(true)}
            className="btn-primary flex items-center gap-2 text-sm"
          >
            <FiZap size={14} /> Ask AI
          </button>
        </motion.div>
        ...
        <motion.div variants={fadeUp} transition={{ duration: 0.35 }} className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-900 dark:text-white">More on your dashboard</h2>
          <button onClick={handleExport} className="btn-secondary flex items-center gap-2 text-sm">
            <FiDownload /> Export CSV
          </button>
        </motion.div>

        <motion.div variants={fadeUp} transition={{ duration: 0.35 }} className="grid sm:grid-cols-3 gap-4">
          <QuickNotesWidget />

          <Link to="/achievements" className="card block hover:border-primary-300 dark:hover:border-primary-500/40 hover:-translate-y-0.5 transition-all">
          ...
          <Link to="/quizzes" className="card block hover:border-primary-300 dark:hover:border-primary-500/40 hover:-translate-y-0.5 transition-all">
            <div className="w-9 h-9 rounded-lg bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-400 flex items-center justify-center mb-3">
              <FiHelpCircle />
            </div>
      ...
      <DoubtSolverModal open={doubtSolverOpen} onClose={() => setDoubtSolverOpen(false)} />
    </AppLayout>
  );
}
